...
object TestGraphExamples {

  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("TestGraphExamples").setMaster("local")
    val sc = new SparkContext(conf)

    // Load the edges as a graph
    val graph = GraphLoader.edgeListFile(sc, "file:/home/orienit/spark/input/graphx/sample.txt")
    ...
    // 4's in neighbors : (1,1)
    // 1's in neighbors : (2,1),(3,1),(4,1),(5,1)
    // 3's in neighbors : (4,1),(5,1)
    // 5's in neighbors : 
    // 2's in neighbors : (1,1)
